...
use plot::{Corner, HLine, Legend, Plot, VLine};
...
impl NoiseGraph {
    ...
    pub fn generate_maxima_lines(
        &mut self,
        values_maxima: &[Value],
        maxima_lines: &mut Vec<VLine>,
    ) {
    ...
    pub fn ui(&mut self, ctx: &egui::CtxRef, _ui: &mut Ui) {
        egui::SidePanel::left("params_panel").show(ctx, |ui| self.params_ui(ui));
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let limit_y_max = HLine::new(self.ng.y_max).color(Color32::from_rgb(140, 140, 140));
            let limit_y_min = HLine::new(self.ng.y_min).color(Color32::from_rgb(140, 140, 140));

            let limit_x_max = VLine::new(self.ng.x_max).color(Color32::from_rgb(140, 140, 140));

            let limit_x_min = VLine::new(self.ng.x_min).color(Color32::from_rgb(140, 140, 140));
        ...
        ComboBox::from_label("Select Noise")
            ...
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut self.selected,
                    NoiseFunctionNames::Sin,
                    NoiseFunctionNames::Sin.to_string(),
                );
                ui.selectable_value(
                    &mut self.selected,
                    NoiseFunctionNames::Cos,
                    NoiseFunctionNames::Cos.to_string(),
                );
                ui.selectable_value(
                    &mut self.selected,
                    NoiseFunctionNames::Square,
                    NoiseFunctionNames::Square.to_string(),
                );
            });
